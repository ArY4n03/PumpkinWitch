...
func _ready():
	...
	var tmp = [] #this will contain score_id and score
	for score in scores:
		tmp.append([score['score_id'],score['score']])

	scores = sort_(tmp,scores)
	...
	if len(scores) > 0: 
		...
		tmp = [] #this will contain score_id and score
		for score in scores:
			tmp.append([score['score_id'],score['score']])
...
#Added this function to sort scores in acesnding order
func sort_(t,scores):
	var s = [] #this will contain sorted scores
	var t1
	var t2
	for j in range(len(t)): #sorting this list by score
	...
	#now we have a list with sorted score
...
func render_board(scores: Array, local_scores: Array) -> void:
	...
	if all_scores.is_empty():
		for score in scores:
			add_item(score.player_name, str( str(int((score.score)/100)) +":" + str((score.score-int(score.score)) * 60)))
	else:
		for score in all_scores:
			add_item(score.player_name,str( str(int((score.score))) +":" + str(round((score.score-int(score.score)) * 60))))
